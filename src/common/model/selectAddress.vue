	...
	export default{
		...
		methods:{
			...
			async provices(){	//获取一级地址列表
				await systemApi.provices().then(res=>{
			...
			async cities(id){	//获取二级地址
				await systemApi.cities(id).then(res=>{
					if(res.data.code==200){
			...
			async counties(id){	//获取三级地址
				await systemApi.counties(id).then(res=>{
					if(res.data.code==200){
			...
			async towns(id){	//获取四级地址
				await systemApi.towns(id).then(res=>{
					if(res.data.code==200){
						this.townList = res.data.data;
						this.level = !res.data.data.length?3:4
					}else{
						uni.showToast({
							title:res.data.message,
							icon:"none"
			...
			async chooseAddr(index,item,flag){	//点击事件触发
				...
				if(index==this.level-1 && !flag){
					let params = {
						...
						townId:this.selectedItems[3].id?this.selectedItems[3].id:"",
						townName:this.selectedItems[3].fullName?this.selectedItems[3].fullName:"",
					}
