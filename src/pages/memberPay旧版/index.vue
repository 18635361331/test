<template>
	<view class="page commonPageTitle">
		...
		<view class="payInfo" :class="{active:chooseType=='微信',isEnterprise:(isEnterprise && chooseType=='微信')}" @click="chooseChange('微信')">
			<view class="payType">
		...
		<!-- #ifndef MP-WEIXIN -->
		<view class="payInfo" v-if="!isWxFlag" :class="{active:chooseType=='支付宝',isEnterprise:(isEnterprise && chooseType=='支付宝')}" @click="chooseChange('支付宝')">
			<view class="payType">
		...
		<!-- #endif -->
		<view class="buyNow">
			<view class="buyNowBtn"  :class="isEnterprise?'active':''" v-if="payPrice" @click="buyNow">
	...
	export default {
		...
		methods: {
			...
			buildPrepPaySign(){	//获取公众号支付相关信息
				payMentApi.buildPrepPaySign({code:this.wxCode,orderPayId:this.$Route.query.orderPayId,orderType:"plus"}).then(res=>{
					if(res.data.code==200){
			...
			buildMiniPrepPaySign(code) { //获取小程序支付相关信息
				payMentApi.buildMiniPrepPaySign({
					...
					orderType: this.$Route.query.enterprise?"company":"plus"
				}).then(res => {
					if (res.data.code == 200) {
			...
			payCallBack(flag){
				if(flag){
					...
					this.$Router.replace({
						path: "/pages/memberPaySuccess/index",
						query: {
							goodsId:this.$Route.query.goodsId?this.$Route.query.goodsId:"",
							enterprise:this.$Route.query.enterprise?true:""
						}
					})
				}else{
					uni.showToast({
