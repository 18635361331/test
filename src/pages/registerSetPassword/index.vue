<template>
	<view class="page commonPageTitle">
		...
		<view class="forgetBox">
			<view class="contain">设置密码</view>
			<view class="everRow">
				<input type="text" v-model="params.password" @input='pswJudge' v-if="pswFlag.first" placeholder="请输入密码"
				 placeholderClass="placeholder" />
				<input type="password" v-model="params.password" @input='pswJudge' v-else placeholder="请输入密码" placeholderClass="placeholder" />
			...
			<view class="everRow aginPsw">
				<input type="text" v-if="pswFlag.second" v-model="aginPsw" placeholder="请再次确认密码" placeholderClass="placeholder" />
				<input type="password" v-else v-model="aginPsw" placeholder="请再次确认密码" placeholderClass="placeholder" />
				<view class="pswIcon" @click="pswFlag.second" :class="pswFlag.second?'active':''"></view>
			</view>
			<view class="pswHint">8-20位，由字母（需同时包含大小写）、数字组成</view>
			<view class="complete" @click="complete">登录</view>
		</view>
		<view class="skip" @click="skipSetMemberPwd">
			跳过此步骤
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			complete() { //完成触发
				...
				this.setMemberPwd() //重置密码
			},
			pswJudge() { //强度校验
				this.pswStrength = passwordJudge.checkPwdIntensity(this.params.password)
			},
			setMemberPwd() { //重置密码
				memberApi.setMemberPwd({
					setPwdCode: this.params.operateCode,
			...
			skipSetMemberPwd(){	//跳过此步骤  
				memberApi.skipSetMemberPwd({skipPwdCode:this.$Route.query.operateCode,telephone:this.$Route.query.telephone}).then(res=>{
					if(res.data.code==200){
						uni.setStorageSync("sessionId",res.data.data.sessionId);
						uni.setStorageSync("userInfo",JSON.stringify(res.data.data));
						this.goBack(2)
