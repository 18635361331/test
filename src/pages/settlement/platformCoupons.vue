<template>
	<view class="page">
		<view class="disCount" @click="openCouponPopup">
			<view>
				<text>平台优惠券</text>
				<text class="couponsNum">{{platforCouponsInfo.availableList.length}}张可用</text>
			</view>
			<view class="discountPrice" v-for='item,index in platforCouponsInfo.availableList' v-if="choosePlatfromId==item.id" :key='index'>
		...
		<uni-popup ref="couponPopup" type="bottom">
			<view class="couponPopup">
				...
				<scroll-view scroll-y="true" class="couponMain">
					<view class="everyRow" v-for='item,index in platforCouponsInfo.availableList' :key='index'>
						...
						<view class="couponBtnIcon" @click="couponChange(item)" :class="choosePlatfromId==item.id?'active':''"></view>
					</view>
					<view class="availableNotUse" @click="clearCoupon">
						不使用优惠券
					</view>
					<view class="unavailable" v-if="platforCouponsInfo.unavailableList.length">
						不可用优惠券
					</view>
					<view class="everyRow" v-for='item,index in platforCouponsInfo.unavailableList' :key='index'>
...
<script>
	export default {
		props: ["platforCouponsInfo","choosePlatfromId"],
		data() {
			return {
		...
		methods: {
			openCouponPopup() { //打开平台优惠券弹窗
				this.$refs.couponPopup.open()
			},
			clearCoupon(){	//不使用优惠券
				this.$emit("couponChange",this.storeId,"")
			},
			closeCouponPopup(){	//关闭平台优惠券弹窗
				this.$refs.couponPopup.close()
			},
			couponChange(item){	//优惠券选择发生变化时触发
				if(this.choosePlatfromId!=item.id){
					this.$emit("couponChange",item.id)
				}
			}
		},
		mounted() {
