...
<script>
	...
	export default {
		...
		onLoad() {
			...
			if(this.$Route.query.priceRangeLeft){	//凑单处理
				this.searchParams.order = "asc";
				this.searchParams.orderBy = "price";
				this.searchParams.priceRangeRight = 999999999;
				this.searchParams.customizeFlag = 0
			}
			this.searchCustom();
		},
		methods: {
			searchCustom(type) { //查询商品列表
				...
				searchApi.searchCustom(this.searchParams, this.pageInfo).then(res => {
					uni.hideLoading();
