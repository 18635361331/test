<template>
	<view class="page giftCardGoods">
		<defaultTitle :dto="defaultTitleDto"></defaultTitle>
		<view class="searchbox">
			<view class="searchInput active">
				<view class="searchIcon"></view>
				<view class="inputBox" @click="goToSearch">{{searchValue?searchValue:"在礼卡适用商品中搜索"}}</view>
		...
		<view class="cardName ellipsis-one">
			以下商品可使用 {{cardName}} 礼卡
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			searchCustom(type) { //查询商品列表
				if(!type){
					uni.showLoading({
						title: "加载中..."
					})
				}
				appApi.getCardItemList(this.$Route.query.batchId,{...this.searchParams, ...this.pageInfo}).then(res => {
					uni.hideLoading();
