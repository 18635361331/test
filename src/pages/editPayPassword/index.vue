<template>
	<view class="page commonPageTitle">
		...
		<view class="surePswBox" v-if="nextFlag==1">
			<view class="verification">输入当前支付密码，以验证身份</view>
			<view class="passwordBox">
				<view class="everyPswBox" v-for="item,index in 6" :key='index'>
					<text v-if="index<=(payPwd?payPwd.length-1:-1)">*</text>
				</view>
				<input type="number" v-model="payPwd" @input='valPayPwd' maxlength="6" >
			</view>
		</view>
		<view class="surePswBox" v-if="nextFlag==2">
			<view class="verification">设置新支付密码，用于提现验证</view>
			<view class="passwordBox">
				<view class="everyPswBox" v-for="item,index in 6" :key='index'>
					<text v-if="index<=(newPayPwd?newPayPwd.length-1:-1)">*</text>
				</view>
				<input type="number" v-model="newPayPwd" @input='newPayPwdChange' maxlength="6" >
			</view>
			<view class="pswHint">
				为了保护您的账号安全，支付密码和支付密码尽量不要相同
			</view>
		</view>
		<view class="surePswBox" v-if="nextFlag==3">
			<view class="verification">请再次确认支付密码</view>
			<view class="passwordBox">
				<view class="everyPswBox" v-for="item,index in 6" :key='index'>
					<text v-if="index<=(aginPayPwd?aginPayPwd.length-1:-1)">*</text>
				</view>
				<input type="number" v-model="aginPayPwd" @input='aginPayPwdChange' maxlength="6" >
			</view>
		...
		methods:{
			...
			valPayPwd(){	//验证支付密码（修改支付密码使用）
				if(this.payPwd.length>=6){
					memberApi.valPayPwd({operateCode:this.operateCode,payPwd:this.payPwd,type:6}).then(res=>{
						if(res.data.code==200){
							this.nextFlag = 2;
							this.operateCode = res.data.data
						}else{
			...
			resetPayPwd(){	//重置支付密码
				memberApi.resetPayPwd({operateCode:this.operateCode,payPassword:this.newPayPwd}).then(res=>{
					if(res.data.code==200){
						uni.showToast({
							title:"密码重置成功",
							icon:"none"
